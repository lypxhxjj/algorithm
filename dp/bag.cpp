...
// 3. dp的含义则根据题目来确定，在满足任意i个元素的序列和小于等于j时的一个最值的性质。
// 4. 那dp[j][i]可以吗？理解上可以的，有顺序要求的是j，当计算j的时候，那么比j小的一定已经计算好了就可以了。i就无所谓。
//
// 为啥背包问题无法直接使用一重循环思考？
// 1. 二重循环的第一重，是某个元素选与不选，遍历n个元素，n个元素的选与不选，那么对于固定的sum(j)而言，就得到了这个j的最优解。
// 2. 没办法去掉第二重，是因为计算当前j，需要用到 j - w[i]，j的变化需要从头开始。
//
// 有了含义，可以解法了：
